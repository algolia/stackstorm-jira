...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([{"value": "Other"}, {"value": "Impediment"}], True),
        (None, False),
    ],
)
...
def test_run_missing_flagged_field_on_issue_error() -> None:
    jira = mock.Mock()
    jira.fields.return_value = [FLAGGED_FIELD]
    jira.issue.return_value.raw = {"fields": {}}
    action = IsFlagged.__new__(IsFlagged)
    action._client = jira
    ...
    assert str(error.value) == (
        'Issue "IAAS-123" does not expose Jira field "customfield_10038"'
    )
